...
> **Related:** Интегрируется с [AI-RULES.md](AI-RULES.md) (Rule 9: Микросервисная архитектура) и [SECURITY-RULES.md](SECURITY-RULES.md) для полного охвата разработки, безопасности и инфраструктуры.
> 
> **Security Notice:** При применении правил контейнеризации обязательно следуйте требованиям SECURITY-RULES (Rules 1,2,3,4,5,8,10) для обеспечения безопасности на уровне инфраструктуры.
...
- 
**Уточнение (Development vs Production):**
- **Dev images:** Допускают bash, debug tools, но явно помечены как `DEV-ONLY` в tags
- **Prod images:** Только необходимые утилиты, no sh, минимальная поверхность атаки
- **CI/CD:** Отдельные образы для build/test и runtime
- **Cross-ref:** SECURITY-RULES Rule 14 (Security by Default)
